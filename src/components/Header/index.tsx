...
const Header = () => {
  ...
  // Sticky Navbar
  // Determine the initial sticky state based on the current scroll position on the client.
  ...
  useEffect(() => {
    // Handler to update the sticky state.
    const onScroll = () => {
      setSticky(window.scrollY >= 80);
    };
    // Register the scroll listener once on mount.
    window.addEventListener("scroll", onScroll);
    // Perform an initial check in case the user has scrolled before hydration.
    onScroll();
    return () => {
          ...
          <div className="relative -mx-4 flex items-center justify-between">
            <div className="w-60 max-w-full px-4 xl:mr-12">
              <Link href="/" className="header-logo block w-full py-5 lg:py-2">
                {/* Remove the arbitrary w-35 utility – rely on width/height props. */}
                <Image
